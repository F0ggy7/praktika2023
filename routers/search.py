...
async def get_search(types: int = Query(..., gt=0, le=2, description='Specify the type of person or company(1 or 2)'), searchString: str = Query("", description='Specify what query to search for') , isActiveLegalCase: str = Query("null", description='Indicate the status of the case (true, false, null) Default is null'), regionId: str = Query("",description="You can specify a region ID")):
